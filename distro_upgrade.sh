#!/bin/bash
# =============================================================================
# 脚本名称: distro_upgrade.sh
# 描    述: Debian / Ubuntu 系统自动逐级升级脚本
#           生产入口支持 Debian 11~13 与 Ubuntu LTS 相邻升级
#           适用于 VPS、物理机、虚拟机等大多数 Linux 环境
# =============================================================================
# 版本历史:
...
#   sudo ./distro_upgrade.sh              # 正常升级（稳定版，默认）
#   sudo ./distro_upgrade.sh --allow-testing  # 允许升级到 Debian 14 Forky
#   sudo ./distro_upgrade.sh --check      # 检查升级状态
#   sudo ./distro_upgrade.sh --fix-grub   # 专门修复 GRUB
#   sudo ./distro_upgrade.sh --help       # 查看完整帮助
# =============================================================================
# 注意事项:
#   - 需要 root 或 sudo 权限
...
# ── 脚本元信息 ────────────────────────────────────────────────────────────────
SCRIPT_VERSION="3.6"
SCRIPT_NAME="distro_upgrade.sh"
SCRIPT_DATE="2026-06-10"
SCRIPT_REPO="https://raw.githubusercontent.com/everett7623/debian-auto-upgrade/main/distro_upgrade.sh"
# GitHub 在国内可能被阻断，提供 CDN 回退地址
SCRIPT_REPO_CDN="https://cdn.jsdelivr.net/gh/everett7623/debian-auto-upgrade@main/distro_upgrade.sh"
...
self_update_mode() {
    ...
    if ! command -v wget >/dev/null 2>&1; then
        ...
        log_error "也可使用 curl 手动下载: curl -fL --proto '=https' --tlsv1.2 -o distro_upgrade.sh $SCRIPT_REPO"
        rm -f "$tmp_file"
        return 1
    fi
