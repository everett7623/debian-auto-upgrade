...
check_pending_upgrade() {
    local upgrade_state_file="/tmp/debian_upgrade_state"
    ...
    if [[ -f "$upgrade_state_file" ]]; then
        ...
        if [[ "$actual_current_version" == "$current_version" ]]; then
            ...
            if [[ -n "$next_version" ]]; then
                ...
                echo "  [N] 不，保持当前版本 Debian $actual_current_version"
                echo "  [C] 检查系统状态但不升级"
                echo "  [Q] 退出脚本"
                echo
...
cleanup_upgrade_state() {
    ...
    if [[ -f "$upgrade_state_file" ]]; then
        rm -f "$upgrade_state_file" 2>/dev/null || true
        log_debug "升级状态文件已清理"
    fi
}
